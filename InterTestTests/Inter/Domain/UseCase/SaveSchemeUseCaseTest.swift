...
class SaveSchemeUseCaseTest: XCTestCase {
    private var sut: SaveSchemeUseCase!
    ...
    func testSaveSchemeUseCase_ResponseSuccess() {
        //Given
        ...
        //When
        cancellable = sut.execute(params: TestDomainConstants.schemeResponse)
            .sink { completion in
    ...
    func testSaveSchemeUseCase_ResponseError() {
        //Given
        let expectationFailure = expectation(description: "failure")
        let expectation = self.expectation(description: "Consume persistence Service Response Expectation")
